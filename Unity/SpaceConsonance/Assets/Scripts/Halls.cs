    ...
    public void PiecePlaced(DragNDrop piece){
        for(int i = 0; i < hallList.Count; i++){
            if(!hallList[i].isBusy){
                if(Mathf.Abs(piece.transform.position.x - hallList[i].transform.position.x) <= 100f && Mathf.Abs(piece.transform.position.y - hallList[i].transform.position.y) <= 100f && hallList[i].transform.eulerAngles.y ==0){
